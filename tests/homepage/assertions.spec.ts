...
test('open the page and asserts homepage', async ({ page }) => {
  ...
  await expect(page.locator('#search_query_top')).toHaveAttribute(
    'type',
    'text'
  );
  await expect(page.locator('.shopping_cart')).toBeVisible();
  await expect(page.getByText('Custom Block')).toBeVisible();
  await expect(page.locator('#facebook_block')).toHaveText(
  ...
  await expect(page.locator('#newsletter-input')).toHaveValue(
    'Enter your e-mail'
  );
});
