...
test('asserts homepage', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.locator('#user_email')).toHaveAttribute('type', 'text');
  await expect(page.locator('#contact-link')).toBeEnabled();
  await expect(page.locator('.icon-truck')).toHaveClass('font-family');
